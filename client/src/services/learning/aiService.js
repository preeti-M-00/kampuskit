...
const chat = async (documentId, message) => {
  try {
    ...
    throw error.response?.data || { message: 'Chat response failed' };
  }
};

const explainConcept = async (documentId, concept) => {
    try{
        const response = await learningAxios.post(API_PATHS.AI.EXPLAIN_CONCEPT, {documentId, concept});
        return response.data?.data;
    } catch(error){
...
const getChatHistory = async (documentId) => {
    try{
        const response = await learningAxios.get(API_PATHS.AI.GET_CHAT_HISTORY(documentId));
        return response.data;
    } catch(error){
